...
int reemplazo_FIFO() {
	...
	entrada_pagina(lista_marcos->region, lista_marcos->pagina)->nmarco = -1;
...
void rellenar_entrada_marco(int nmarco, int reg, int pag) {
	...
	entrada_pagina(reg, pag)->nmarco = nmarco;
}
...
entrada_tabla_paginas *leer_entrada_marco(int nmarco) {
	...
	assert((nmarco>=0) && (nmarco<tabla_marcos.tam) &&
		...
		pagina = entrada_pagina(tabla_marcos.marcos[nmarco].region, tabla_marcos.marcos[nmarco].pagina);
